...
- Optional `VOICEMAIL_CONF` env var: when set to an Asterisk/FreePBX voicemail.conf path, extension and email are loaded from that file instead of `EXTENSIONS_JSON` (or CSV). Parses context sections for mailbox lines (`extension=password,name,email,...`); uses the third field as email (first address if pipe-separated). Intended for running the app on the Asterisk/FreePBX server.
